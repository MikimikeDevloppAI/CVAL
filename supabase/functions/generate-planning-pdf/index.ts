...
serve(async (req) => {
  ...
  try {
    ...
    if (fileObj.Url) {
      console.log('PDF generated by ConvertAPI (URL):', fileObj.Url);
      const pdfResponse = await fetch(fileObj.Url);
      const pdfBlob = await pdfResponse.blob();
      pdfBuffer = await pdfBlob.arrayBuffer();
    } else if (fileObj.FileData) {
      console.log('PDF generated by ConvertAPI (inline base64), decoding...');
      const binaryStr = atob(fileObj.FileData);
      const len = binaryStr.length;
      const bytes = new Uint8Array(len);
      for (let i = 0; i < len; i++) bytes[i] = binaryStr.charCodeAt(i);
      pdfBuffer = bytes.buffer;
    } else {
      console.error('Invalid ConvertAPI response (no Url or FileData):', JSON.stringify(convertData));
