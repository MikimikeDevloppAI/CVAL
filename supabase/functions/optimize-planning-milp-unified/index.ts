...
serve(async (req) => {
  ...
  try {
    ...
    if (personnelToInsert.length > 0) {
      ...
      // ============================================================
      // APPEL DE L'EDGE FUNCTION DE SWAP
      // ============================================================
      console.log("\n🔄 ========== PHASE 2 : OPTIMISATION SWAP ==========");
      console.log(`Préparation du payload pour ${cleaned.length} assignations...`);
      
      // Préparer le payload pour la phase swap
      const swapPayload = {
        planning_id,
        selected_dates,
        week_start,
        ...
        secretaires,
        besoinsEffectifs,
        secretairesSitesMap: Array.from(secretairesSitesMap.entries()).map(([k, v]) => ({ key: k, value: v })),
        secretairesMedecinsMap: Array.from(secretairesMedecinsMap.entries()).map(([k, v]) => ({ key: k, value: v })),
        capacitesMap: Array.from(capacitesMap.entries()).map(([k, v]) => ({ key: k, value: v }))
      };

      // Appeler la nouvelle edge function de swap
      console.log("🚀 Appel de l'edge function optimize-planning-swap...");
      const { data: swapResult, error: swapError } = await supabase.functions.invoke(
        "optimize-planning-swap",
        { body: swapPayload }
      );

      if (swapError) {
        console.error("❌ Erreur pendant le swap:", swapError);
        console.log("⚠️ FALLBACK: Insertion directe sans swap");
        
        // Fallback: insérer directement sans swap
        const { error: persError } = await supabase
          .from("planning_genere_personnel")
          .insert(cleaned);
        
        if (persError) {
          console.error("Erreur insertion personnel (fallback):", persError);
          throw persError;
        }
        
        console.log(`✓ ${cleaned.length} assignations personnel insérées (sans swap)`);
      } else {
        console.log("✅ Swap terminé avec succès:", swapResult);
        console.log(`   → ${swapResult.totalSwaps} échanges appliqués`);
        console.log(`   → Gain total: +${swapResult.totalGain}`);
        console.log(`   → ${swapResult.assignmentsCount} assignations insérées`);
      }
    } else {
