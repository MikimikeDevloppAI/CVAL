...
function optimizePeriod(
  ...
  // 2. Créer les créneaux possibles pour chaque secrétaire (sans filtrage Top-K)
  for (const secretaire of secretairesDispos) {
    ...
    for (const [besoinKey, besoin] of besoinsParSite) {
      ...
      model.variables[varName] = {
        objective: -1,  // Négatif pour encourager les assignations site
    ...
    model.variables[adminVarName] = {
      objective: +PENALTY_ADMIN_BASE / Math.pow(2, adminCount),  // Pénalité positive, décroît pour équilibrer
