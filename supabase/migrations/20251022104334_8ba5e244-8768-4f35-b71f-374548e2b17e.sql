...
-- Recreate the materialized view with descriptive site_nom for operating room needs
...
besoins_bloc AS (
  SELECT
    ...
    END AS periode,
    sb.id AS site_id,
    -- Descriptive site name: Type intervention - Doctor - Need
    ti.nom || ' - Dr. ' || COALESCE(m.name || ' ' || m.first_name, 'Non assigné') || ' - ' || bo.nom AS site_nom,
  ...
  JOIN public.types_intervention_besoins_personnel tibp 
    ...
    AND tibp.actif = true
  JOIN public.types_intervention ti 
    ON ti.id = pgbo.type_intervention_id
  LEFT JOIN public.medecins m 
    ON m.id = pgbo.medecin_id
  JOIN public.besoins_operations bo 
    ON bo.id = tibp.besoin_operation_id
