...
-- Recreate the materialized view with corrected logic for both sites and operating room
...
besoins_bloc AS (
  ...
  JOIN public.types_intervention_besoins_personnel tibp 
    ...
    AND tibp.actif = true
  JOIN public.sites s ON s.nom = 'Clinique La Vallée - Bloc opératoire'
  WHERE pgbo.statut != 'annule'
    AND pgbo.validated = false
),
tous_besoins AS (
