-- Modifier la table horaires_base_secretaires pour utiliser le même système que les médecins
-- Gérer le cas où les colonnes existent déjà ou non

-- Supprimer l'ancienne colonne alternance_semaine_reference si elle existe
DO $$ 
BEGIN
  IF EXISTS (
    SELECT 1 FROM information_schema.columns 
    WHERE table_schema = 'public' 
    AND table_name = 'horaires_base_secretaires' 
    AND column_name = 'alternance_semaine_reference'
  ) THEN
...
-- Ajouter la colonne alternance_semaine_modulo si elle n'existe pas
DO $$ 
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM information_schema.columns 
    WHERE table_schema = 'public' 
    AND table_name = 'horaires_base_secretaires' 
    AND column_name = 'alternance_semaine_modulo'
  ) THEN
    ALTER TABLE public.horaires_base_secretaires
    ADD COLUMN alternance_semaine_modulo integer DEFAULT 0;
  END IF;
END $$;

-- Mettre à jour la fonction handle_horaire_secretaire_insert_logic pour utiliser le modulo
...
-- Mettre à jour la fonction recreate_secretary_capacite pour utiliser le modulo
