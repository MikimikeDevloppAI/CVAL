...
-- Recreate the materialized view excluding bloc operatoire from site needs
...
WITH besoins_sites AS (
  ...
  WHERE be.type = 'medecin'
    ...
    -- Exclure le bloc opératoire des besoins sites
...
SELECT
  ...
  COALESCE(MAX(ca.nombre_assignes), 0) AS nombre_assignes,
  GREATEST(0, SUM(tb.nombre_besoins) - COALESCE(MAX(ca.nombre_assignes), 0)) AS nombre_manquant
FROM tous_besoins tb
LEFT JOIN capacites_assignees ca 
  ON ca.date = tb.date 
  AND ca.periode = tb.periode 
  AND (
    -- Pour les sites normaux : matcher par site_id
    (tb.type_besoin = 'site' AND ca.site_id = tb.site_id)
    OR
    -- Pour le bloc opératoire : matcher uniquement par planning_genere_bloc_operatoire_id ET besoin_operation_id
...
HAVING GREATEST(0, SUM(tb.nombre_besoins) - COALESCE(MAX(ca.nombre_assignes), 0)) > 0
